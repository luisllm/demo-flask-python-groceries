...
def remove_items():
    ...
    for item in checked_boxes:
        # If the item exists on the shopping_list, then remove the item
        if item in session["shopping_items"]:
            # Get the index of that item in the shopping list
...
def get_db():
    ...
    if db is None:
        ...
        # It selects only 5 items to be in the shopping list
